...
import { updateQuotationTotals, formatCurrency } from '@/lib/quotation-utils';
...
export default function EditQuotationPage() {
  ...
  const { savedQuotations, saveQuotation, predefinedModules, savedClients } = useQuotation();
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <h2 className="text-3xl font-bold text-foreground mb-2">Edit Quotation</h2>
        <p className="text-muted-foreground">{quotation.referenceNumber}</p>
      </div>
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="font-semibold text-foreground">{formatCurrency(updated.discountAmount)}</p>
          </div>
          <div>
      ...
      <div className="flex gap-3">
        <Link
          href={`/dashboard/quotations/${quotation.id}`}
          className="flex-1 px-6 py-3 border border-primary text-primary rounded-lg hover:bg-primary/10 transition-colors font-medium text-center"
        >
          Cancel
        </Link>
        <button
